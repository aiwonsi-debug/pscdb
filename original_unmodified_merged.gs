 ...
 *   PSC_SYNC_TOKEN            = ... (Optional shared token for relay sync)
...
function doGet() {
  return json_({ ok: true, service: 'PSC WDB LINE to Sheets gateway', version: 2 });
}
...
function doPost(e) {
  ...
  try {
    ...
    // 1. ตรวจจับคำสั่งส่งตรงจาก LINE Messaging API Webhook
    if (body.events) {
      // 1.1 ถ้าเป็นการกดปุ่ม Verify จาก LINE Console (events ว่าง)
      if (!body.events || body.events.length === 0) {
        return json_({ ok: true, status: 'verified' });
      }

      // 1.2 วนลูปประมวลผลข้อความจริงจาก LINE (ทั้งข้อความตัวอักษรและรูปภาพ PO)
    ...
    // 2. ตรวจจับคำสั่งส่งผ่าน Relay / Backend Sync เดิม (ต้องใช้ PSC_SYNC_TOKEN)
    const payload = body.payload || {};
    const reportId = String(payload.reportId || '');
    const syncJobId = String(payload.syncJobId || '');
    const messageId = String(payload.messageId || '');

    if (!isAuthorized_(e, body)) {
      auditBestEffort_([receivedAt, 'unauthorized', reportId, syncJobId, messageId, 'Invalid or missing PSC_SYNC_TOKEN']);
      return json_({ ok: false, error: 'unauthorized' });
    }

    if (body.event !== 'line_report' || !body.payload) {
      auditBestEffort_([receivedAt, 'rejected', reportId, syncJobId, messageId, 'event must be line_report and payload is required']);
    ...
    try {
      ...
      const duplicate = findDuplicate_(reportSheet, messageId, reportId);

      if (duplicate) {
        auditSheet.appendRow([
          receivedAt, 'duplicate', reportId, syncJobId, messageId,
          'Already received; no duplicate report row written'
        ]);
        return json_({ ok: true, duplicate: true, reportId: reportId });
      }

      // หมายเหตุ: syncedAt คงเป็น Date object ตรง ๆ (ไม่ผ่าน cleanCell_) เพื่อให้ชีตอ่านเป็นวันที่ได้
      reportSheet.appendRow(REPORT_HEADERS.map(function (key) {
        return key === 'syncedAt' ? receivedAt : cleanCell_(body.payload[key]);
      }));
      auditSheet.appendRow([
        receivedAt, 'accepted', reportId, syncJobId, messageId,
        'Report appended to LINE Reports'
      ]);
      return json_({ ok: true, duplicate: false, reportId: reportId });
    } finally {
      lock.releaseLock();
    }
  } catch (err) {
...
function handleLineDirectEvent_(event, receivedAt) {
  ...
  const KEYWORD_PATTERN = /(กะหล่ำ|หอม|แครอท|มันม่วง|มันส้ม|มันเหลือง|ผัก|สต็อก|stock|ขึ้นของ|ส่งมอบ|ยกเลิก|cancel|ราคา|ค่ารถ|บาท|พันธุ์ช้าง|สวน|ตัน|กก|โล|ถุง|ศาลายา|บ่อสลี|ป้าผา|เฮียหนิง|เจ๊อารีย์|เฮียบุญชู|เจ๊พรรณี|เฮียธงชัย|พี่บู้|พี่อั๋น|ปากคลอง|นิ่มซี่เส็ง|รถ|ทะเบียน|โรงงาน)/i;
  if (!KEYWORD_PATTERN.test(rawText)) {
    Logger.log("Skipped casual message: " + rawText);
    return;
  }

  // เรียก Gemini แปลงภาษาพูดเป็น JSON
...
/**
 * API สำหรับให้ pscdb.onrender.com หรือหน้าเว็บอื่นเรียกดูข้อมูลสดตรงจาก Google Sheets ทั้ง 4 แท็บ
 */
function doGet(e) {
  ...
  try {
    const PHYSICAL_STOCK_ID = "12nZjwTQQrk6mDJMsaw2pZxYdi8MubippOevZyzXquIw";
    const FARM_OPS_ID = "195Foz8mjcLt1q5agCh28FoyJkg4VxGhMt86XqX7ZSCM";
    const MATERIAL_PRICES_ID = "1-2n4Q2XYjGyRqoogAnS_Id1tGzUBq2bKHvHVKeXEYXM";
    ...
    // 2. ดึงข้อมูลคิวงานสั่งรถล่วงหน้า
    ...
    // 3. ดึงข้อมูลราคาวัตถุดิบและค่ารถ
    ...
    const priceRows = cleanPrice.getDataRange().getValues();
      ...
      // 5. ข้อความคุยเล่น ทักทาย หรือไม่ใช่รายงาน -> ข้ามอย่างเงียบๆ ไม่ตอบ ไม่บันทึก ไม่รบกวนใคร
...
function handleLineImageEvent_(event, receivedAt) {
  ...
  try {
    ...
    // 1. ดาวน์โหลด Binary Content ของรูปภาพจาก LINE Messaging API
    ...
    if (response.getResponseCode() !== 200) {
      ...
      auditSheet.appendRow([receivedAt, 'error', reportId, syncJobId, messageId, 'Download image failed: HTTP ' + response.getResponseCode()]);
      return;
    }
    ...
    // 2. ส่งรูปให้ Gemini 2.5 Flash ช่วยอ่านตารางและตัวเลขใน PO
    ...
    if (!parsed || parsed.reportType === 'none') {
      ...
      auditSheet.appendRow([receivedAt, 'ignored', reportId, syncJobId, messageId, 'Non-order image, not a report']);
      return;
    }

    // 3. บันทึกและส่งข้อมูลเข้าชีตเป้าหมาย
    ...
    Logger.log("handleLineImageEvent error: " + imgErr);
    auditBestEffort_([receivedAt, 'error', reportId, syncJobId, messageId, 'Image process error: ' + String(imgErr)]);
  } finally {
    lock.releaseLock();
  }
}
...
function interpretImageWithGemini(base64Data, mimeType) {
  ...
  return callGeminiApi_(prompt, { mimeType: mimeType, base64Data: base64Data }, apiKey) || { reportType: 'none' };
}
...
function callGeminiApi_(promptText, imagePart, apiKey) {
  ...
  if (imagePart) {
    parts.push({ inlineData: { mimeType: imagePart.mimeType, data: imagePart.base64Data } });
  }
  ...
  try {
    ...
    const errDetail = "Gemini API HTTP " + res.getResponseCode() + ": " + res.getContentText().slice(0, 200);
    Logger.log(errDetail);
    auditBestEffort_([new Date(), 'error', '', '', '', errDetail]);
  } catch (err) {
    Logger.log("Gemini exception: " + err);
    auditBestEffort_([new Date(), 'error', '', '', '', 'Gemini exception: ' + String(err)]);
  }
  return null;
}
...
function isAuthorized_(e, body) {
  const expected = PropertiesService.getScriptProperties().getProperty('PSC_SYNC_TOKEN');
  if (!expected) return false;
  const provided = String(
    body.token || (e && e.parameter && e.parameter.token) || ''
  );
...
function validatePayload_(payload) {
  ...
  if (!String(payload.reportId || '').trim()) throw new Error('payload.reportId is required');
  if (!String(payload.syncJobId || '').trim()) throw new Error('payload.syncJobId is required');
  if (!String(payload.rawText || '').trim()) throw new Error('payload.rawText is required');
  if (String(payload.rawText).length > MAX_RAW_TEXT_LENGTH) throw new Error('payload.rawText is too long');
}
