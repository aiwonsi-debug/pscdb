...
const startIdx = code.indexOf('const isStockPattern = !hasNegation &&');
const endIdx = code.indexOf('// 3.0 Anti-Hallucination Ground-Truth Verification Command');
...
ห้ามมโนข้อมูล ถ้าช่องไหนไม่มีในข้อความให้ใส่ null ห้ามตอบข้อความอื่นนอกจาก JSON\`;
    ...
    code = code.substring(0, startIdx) + unifiedCode + code.substring(endIdx);
    fs.writeFileSync('E:/agy/bot.js', code, 'utf8');
    console.log('Successfully replaced old parsing blocks with Unified AI Engine in bot.js');
} else {
