...
public class FastMsg750Downloader : IDisposable {
    ...
    if ($boundaryMatch.Success) {
        ...
        foreach ($p in $parts) {
            ...
            if ($fnMatch.Success) {
                ...
                Write-Output "Msg 750 Attachment filename: $filename"
                $contentIdx = $p.IndexOf("`r`n`r`n")
                if ($contentIdx -ge 0) {
                    $b64 = ($p.Substring($contentIdx + 4).Trim()) -replace '\s+', ''
                    $fileBytes = [System.Convert]::FromBase64String($b64)
                    $destPath = Join-Path "E:\รวมงาน\งาน 25-26\Siam Yamamori\PO\Sep" $filename
                    [System.IO.File]::WriteAllBytes($destPath, $fileBytes)
                    Write-Output ">>> SAVED TO SEP: $destPath ($($fileBytes.Length) bytes)"
                }
            }
        }
    }
