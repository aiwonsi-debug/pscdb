...
public class YamamoriSepDownloaderV2 : IDisposable {
...
try {
    ...
    $searchRes = $client.Exec('SEARCH SINCE 20-Aug-2026')
    $msgIds = @()
    ...
    Write-Output "Scanning $($msgIds.Count) recent messages for September Yamamori POs..."
    ...
    foreach ($mid in $msgIds) {
        ...
        if ($header -match '6908' -or $header -match '6909' -or $header -match 'Yamamori' -or $header -match '2357' -or $header -match '2358' -or $header -match 'FW:\s*\d+' -or $header -match 'FW:\s*\[\s*\d+') {
                ...
                if ($boundaryMatch.Success) {
                    ...
                    foreach ($p in $parts) {
                        ...
                        if ($fnMatch.Success) {
                            ...
                            if ($filename -match '\.pdf$' -and ($filename -match '^\d+' -or $filename -match '^PO\d+')) {
                                ...
                                if ($contentIdx -ge 0) {
                                    ...
                                    try {
                                        ...
                                        if ($fileBytes.Length -gt 1000) {
                                            ...
                                            Write-Output ">>> [DOWNLOADED] $filename -> Sep/ ($($fileBytes.Length) bytes)"
                                        }
                                    } catch {}
                                }
                            }
