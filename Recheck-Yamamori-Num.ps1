...
Write-Host "== RECHECK Gmail: Yamamori Numeric Subject POs (20-Jul-2026 to 31-Aug-2026) ==" -ForegroundColor Cyan
...
public class RecheckImapYNum : IDisposable {
...
if (-not ([System.Management.Automation.PSTypeName]'RecheckImapYNum').Type) {
    Add-Type -TypeDefinition $src
}

$imap    = New-Object RecheckImapYNum
$results = @()
...
try {
    ...
    Write-Host "[*] Searching emails: SINCE 20-Jul-2026 BEFORE 01-Sep-2026 ..." -ForegroundColor Yellow
    $ids = $imap.Search("SINCE 20-Jul-2026 BEFORE 01-Sep-2026")
    Write-Host "[*] Total in range: $($ids.Count) emails - checking Numeric/Yamamori subjects..." -ForegroundColor Yellow
    ...
    foreach ($id in $ids) {
        ...
        $subj = if ($sm.Success) { $sm.Groups[1].Value.Trim() } else { "" }
        $from = if ($fm.Success) { $fm.Groups[1].Value.Trim() } else { "" }
        $ds   = if ($dm.Success) { $dm.Groups[1].Value.Trim() } else { "" }

        # Check if Subject is numeric (e.g. "2282", "2224-2226", "2323", "2357 2358", etc.) or Yamamori
        $isNumericSubj = ($subj -match '^\s*[\d\s,-]+\s*$') -or ($subj -match '\b\d{4}\b') -or ($from -match '(?i)yamamori')
        
        # Skip other known customers (TNS, AFT, Oishi) unless it's Yamamori
        if ($from -match '(?i)nisshin|nattapat|ajinomoto|warisara|oishi') {
            continue
        }

        if (-not $isNumericSubj) { continue }
        ...
        Write-Host "  [MATCH #$matchCount] $dlbl | Subj: '$subj' | From: $from" -ForegroundColor Yellow
...
Write-Host " YAMAMORI NUMERIC PO RECHECK SUMMARY (20-Jul to 31-Aug 2026)" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

if ($results.Count -eq 0) {
    Write-Host "  (No matching emails found)" -ForegroundColor Yellow
} else {
    foreach ($r in ($results | Sort-Object Date)) {
        ...
        Write-Host "  $ico $($r.Date) | Subj: $($r.Subj) | File: $($r.File)"
        if ($r.Path) { Write-Host "         -> Path: $($r.Path)" -ForegroundColor DarkGray }
    }
}
...
Write-Host "  New files saved : $cnew"
Write-Host "  Updated files   : $cupd"
Write-Host "  Existing (same) : $csame"
